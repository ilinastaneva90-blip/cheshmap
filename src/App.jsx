...
const BlueMarkerIcon = L.icon({
    iconUrl: createSvgMarkerIcon('#2563eb'),
    iconSize: [38, 38],
    iconAnchor: [19, 38],
    popupAnchor: [0, -38],
...
const RedMarkerIcon = L.icon({
    iconUrl: createSvgMarkerIcon('#dc2626'),
    iconSize: [42, 42],
    iconAnchor: [21, 42],
    popupAnchor: [0, -42],
    shadowUrl: 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/0.7.7/images/marker-shadow.png',
            ...
            {step === 2 && (
                <div className="absolute bottom-24 right-4 text-white max-w-xs flex flex-col items-end">
                    <div className="bg-blue-600 p-4 rounded-xl shadow-xl border-2 border-white/30 mb-2 text-right animate-pulse-slow origin-bottom-right">
                        ...
                        <p className="text-sm opacity-90">Натиснете тук, за да намерите най-близката чешма до вас и да започнете приключението.</p>
                    </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
                    <div className="bg-gradient-to-br from-yellow-400 to-orange-500 text-white p-8 rounded-3xl shadow-2xl max-w-sm border-4 border-white animate-in zoom-in duration-500 relative overflow-hidden">
                        ...
                        <button 
                            ...
                            className="bg-white text-orange-600 px-8 py-3 rounded-full font-bold text-lg shadow-lg hover:scale-105 transition-transform relative z-10"
                        >
                            Готов съм! 🚀
                        </button>
                    </div>
    ...
    <div className="fixed inset-0 z-50 bg-gradient-to-br from-cyan-900 via-blue-900 to-slate-900 text-white flex flex-col items-center justify-between p-6 text-center animate-in fade-in duration-1000 overflow-y-auto">
      ...
      <div className="w-full pb-4">
        <p className="text-[11px] text-cyan-200/60 font-light flex flex-col items-center gap-1">
            ...
            <span className="text-sm text-cyan-100">"Водата дарява живот"</span>
            <span>на Маргарита и Алекси Димитрови</span>
        </p>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const findNearestFountain = () => {
    ...
    navigator.geolocation.getCurrentPosition(
        ...
        (error) => { console.error(error); alert("Моля, разрешете достъпа до GPS от настройките на браузъра."); setFindingNearest(false); }
    );
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 text-slate-800 font-sans">
        ...
        <div className="flex justify-between items-center max-w-md mx-auto w-full">
          <div className="flex items-center gap-2">
            <div className="bg-white p-1.5 rounded-full">
               <CheshMapLogo className="text-blue-600 w-6 h-6" />
            </div>
            <h1 className="text-xl font-bold tracking-wide">CheshMap</h1>
          </div>
          <div className="text-sm font-medium bg-blue-700 px-3 py-1 rounded-full">{foundCount} / {fountains.length} открити</div>
        </div>
      </header>
      ...
      <main className="flex-1 relative overflow-hidden">
        {activeTab === 'map' && (
          <div className="h-full w-full relative">
            <MapContainer center={[41.6167, 25.0167]} zoom={14} style={{ height: "100%", width: "100%" }} zoomControl={false}>
              ...
              {userLocation && (
                <CircleMarker center={userLocation} pathOptions={{ color: 'white', fillColor: '#2563eb', fillOpacity: 1 }} radius={8}>
                    ...
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-xs text-green-600 font-bold uppercase tracking-wider flex items-center gap-1">
                                <CheckCircle size={14} /> УСПЕХ!
                            </p>
                            <h3 className="text-lg font-bold text-gray-800">Открихте: {scanResult.name}</h3>
                            <p className="text-sm text-gray-500 mt-1">Добавена е към колекцията ви!</p>
                        </div>
            ...
            {nearestResult && (
                <div className="absolute top-4 left-4 right-4 bg-white/95 backdrop-blur shadow-xl rounded-xl p-4 border-l-4 border-red-500 z-[1000] animate-in fade-in slide-in-from-top-4 duration-500 max-w-md mx-auto">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-xs text-gray-500 font-bold uppercase tracking-wider">Най-близката чешма е:</p>
                            <h3 className="text-lg font-bold text-red-600">{nearestResult.name}</h3>
                            <p className="text-sm text-gray-600 mt-1">Разстояние: <strong>{nearestResult.dist} км</strong></p>
                        </div>
                        <button onClick={() => setNearestResult(null)} className="p-1 hover:bg-gray-100 rounded-full"><X size={20} className="text-gray-400" /></button>
                    </div>
                </div>
            )}
            ...
            <button onClick={findNearestFountain} disabled={findingNearest} className="absolute bottom-6 right-4 z-[999] bg-white text-blue-600 p-3 rounded-full shadow-xl border border-blue-100 active:scale-95 transition-all flex items-center gap-2 font-bold text-sm">
                <Compass className={`w-6 h-6 ${findingNearest ? 'animate-spin' : ''}`} />
                {findingNearest ? 'Търсене...' : 'Най-близка'}
            </button>
          </div>
        )}

        {activeTab === 'list' && (
          <div className="p-4 overflow-y-auto h-full space-y-6 pb-20 max-w-md mx-auto w-full">
            <h2 className="text-lg font-bold text-slate-700 mb-2 pl-1">Всички обекти ({fountains.length})</h2>
            {fountains.map(fountain => (
              <div key={fountain.id} className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden flex flex-col">
                ...
                <div className="p-5">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {fountain.isFound && <span className="text-green-500 bg-green-50 px-2 py-1 rounded text-xs font-bold border border-green-100 shrink-0 ml-2">ОТКРИТА</span>}
                  </div>
                  <div className="flex flex-wrap gap-2 mb-3">
                     {fountain.features?.map((feat, i) => (<span key={i} className="text-[10px] font-medium bg-gray-100 text-gray-600 px-2 py-1 rounded border border-gray-200">{feat}</span>))}
                  </div>
                  <p className="text-sm text-gray-500 mb-5 leading-relaxed">{fountain.description}</p>
                  <button onClick={() => { setActiveTab('map'); setFlyToCoords(fountain.coords); }} className="w-full bg-blue-50 hover:bg-blue-100 text-blue-700 text-sm font-medium py-3 rounded-lg flex items-center justify-center gap-2 border border-blue-100 transition-colors">
                      <Navigation size={18} /> Навигирай до там
                   </button>
                </div>
              </div>
            ))}
        ...
        {activeTab === 'reward' && (
            <div className="p-6 h-full flex flex-col items-center justify-center text-center bg-white">
                ...
                <div className="w-full max-w-xs bg-gray-100 rounded-full h-6 mb-3 overflow-hidden border border-gray-200">
                    <div className="bg-gradient-to-r from-blue-500 to-blue-400 h-full transition-all duration-1000 ease-out" style={{ width: `${(foundCount / fountains.length) * 100}%` }}></div>
                </div>
      ...
      <nav className="bg-white border-t border-gray-200 flex justify-around p-2 pb-safe shadow-[0_-5px_15px_rgba(0,0,0,0.03)] z-20 max-w-md mx-auto w-full">
        <button onClick={() => setActiveTab('map')} className={`flex flex-col items-center p-2 rounded-xl transition-all ${activeTab === 'map' ? 'text-blue-600 scale-105' : 'text-gray-400'}`}>
          <Map size={24} strokeWidth={activeTab === 'map' ? 2.5 : 2} /><span className="text-[10px] font-medium mt-1">Карта</span>
        </button>
        <button onClick={() => setActiveTab('list')} className={`flex flex-col items-center p-2 rounded-xl transition-all ${activeTab === 'list' ? 'text-blue-600 scale-105' : 'text-gray-400'}`}>
          <List size={24} strokeWidth={activeTab === 'list' ? 2.5 : 2} /><span className="text-[10px] font-medium mt-1">Списък</span>
        </button>
        <button onClick={() => setActiveTab('reward')} className={`flex flex-col items-center p-2 rounded-xl transition-all ${activeTab === 'reward' ? 'text-blue-600 scale-105' : 'text-gray-400'}`}>
          <Gift size={24} strokeWidth={activeTab === 'reward' ? 2.5 : 2} /><span className="text-[10px] font-medium mt-1">Награда</span>
        </button>
      </nav>
    </div>
  );
